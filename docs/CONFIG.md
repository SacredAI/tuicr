...
| `keymap` | `"vim"` | Which editor's keybindings drive the TUI: `"vim"` or `"helix"`. Switch at runtime with `:set keymap helix`. See [KEYBINDINGS.md](KEYBINDINGS.md#helix-keymap). |
...
Per forge, `merge_method` maps onto:

| `merge_method` | GitHub (`gh pr merge`) | GitLab (`glab mr merge`) |
|----------------|------------------------|--------------------------|
| `squash` | `--squash` | `--squash` |
| `merge` | `--merge` | no flag (merge commit) |
| `rebase` | `--rebase` | `--rebase` |

GitLab has no rebase-merge. Its `--rebase` rebases the source branch onto the
target and then merges it, which is the closest equivalent.

When the merge is blocked only by checks that have not finished, tuicr does
not fail. It asks the forge to merge once they pass — `--auto` on GitHub,
`--when-pipeline-succeeds` on GitLab — and the status bar says `auto-merge
armed` rather than `merged`. Conflicts and unsatisfied branch protection rules
are reported as failures instead, with no retry.

If the review is published but the merge fails, you see both facts in one
message. The comments are already posted, so the submit is not retried.
